...
from tacker.vnfm.infra_drivers.freezer import freezer_client
...
class OpenStack_Driver(abstract_vim_driver.VimAbstractDriver,
    ...
    print("#$############################################### get_freezer_client in openstack_driver #############################")
    print("#$############################################### get_freezer_client in openstack_driver #############################")
    def get_freezer_client(self,auth_dict):
        if not auth_dict:
            LOG.warning(_("auth dict required to instantiate Freezer client"))
            raise EnvironmentError('auth dict required for'
                                   ' mistral workflow driver')
        return freezer_client.FreezerClient(
            keystone.Keystone().initialize_client('2', **auth_dict),auth_dict)
